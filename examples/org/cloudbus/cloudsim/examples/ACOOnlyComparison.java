...
public class ACOOnlyComparison {

    public static void main(String[] args) {
        try {
            System.out.println("Starting ACO Only...");
            ...
            List<Vm> vmList = createVMs(broker.getId(), 5, 1000);
            List<Cloudlet> cloudletList = createCloudlets(broker.getId(), 20);

            broker.submitVmList(vmList);
            broker.submitCloudletList(cloudletList);

            // Basic ACO (random biased scheduling)
            Random rand = new Random();
            for (Cloudlet cl : cloudletList) {
                int vmId = rand.nextInt(vmList.size());
                cl.setVmId(vmId);
            }
    ...
    static Datacenter createDatacenter() throws Exception {
        ...
        for (int i = 0; i < 8; i++) {
            peList.add(new Pe(i, new org.cloudbus.cloudsim.provisioners.PeProvisionerSimple(1000)));
        }
